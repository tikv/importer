...
impl<Client: ImportClient> ImportJob<Client> {
    pub fn new(cfg: Config, client: Client, engine: Engine) -> ImportJob<Client> {
        let speed_limit = Limiter::new(cfg.upload_speed_limit.0 as f64);
        ImportJob {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let import_thread_pool = ThreadPoolBuilder::new()
            .name_prefix("import-job-")
            .pool_size(self.cfg.num_import_jobs)
            .create()
            .unwrap();
        let split_thread_pool = ThreadPoolBuilder::new()
            .name_prefix("dispatch-job-")
            .pool_size(self.cfg.num_import_jobs * 2)
            .create()
            .unwrap();
        ...
        for i in 0..MAX_RETRY_TIMES {
            ...
            let handles = self
                ...
                .await;
            if let Some(e) = handles.filter_map(|x| future::ready(x.err())).next().await {
                res = Err(e)
            }
    ...
    fn new_import_thread(
        ...
        async move {
            ...
            job.run_sub_import_job(retry_ranges).await
        }
    }
    ...
    fn new_split_thread(
        ...
        async move {
            ...
            'NEXT_RANGE: while let Ok(range) = range_rx.recv().await {
                ranges_handled += 1;
    ...
    async fn run_import_threads(
        &self,
        ranges: Vec<Range>,
        retry_ranges: Arc<Mutex<Vec<Range>>>,
        import_thread_pool: &ThreadPool,
        split_thread_pool: &ThreadPool,
    ) -> FuturesUnordered<RemoteHandle<Result<()>>> {
        ...
        for id in 0..self.cfg.num_import_jobs {
            handles.push(
                import_thread_pool
                    .spawn_with_handle(self.new_import_thread(
                        id as u64,
                        sst_rx.clone(),
                        Arc::clone(&retry_ranges),
                    ))
        ...
        for id in 0..split_thread_count {
            handles.push(
                split_thread_pool
                    .spawn_with_handle(self.new_split_thread(
                        id as u64,
                        Arc::clone(&client),
                        range_rx.clone(),
                        sst_tx.clone(),
...
impl<Client: ImportClient> SubImportJob<Client> {
    ...
    async fn run_sub_import_job(&self, retry_ranges: Arc<Mutex<Vec<Range>>>) -> Result<()> {
        let sub_id = self.id;
...
struct ImportSSTJob<'a, Client> {
    ...
    sst: SSTFile,
    client: Arc<Client>,
    speed_limit: &'a Limiter,
}

impl<'a, Client: ImportClient> ImportSSTJob<'a, Client> {
    fn new(tag: String, sst: SSTFile, client: Arc<Client>, speed_limit: &'a Limiter) -> Self {
        ImportSSTJob {
            tag,
            sst,
            client,
    ...
    async fn run_import_sst_job(&mut self) -> Result<()> {
        let start = Instant::now();
        ...
        for i in 0..MAX_RETRY_TIMES {
            ...
            let mut region = match self.client.get_region(range.get_start()).await {
                Ok(region) => {
    ...
    async fn import(&mut self, mut region: RegionInfo) -> Result<()> {
    ...
    async fn upload(&self, region: &RegionInfo) -> Result<()> {
        ...
        for peer in region.get_peers() {
            ...
            if let Err(e) = self.client.upload_sst(store_id, upload).await {
    ...
    async fn ingest(&self, region: &RegionInfo) -> Result<()> {
        let start = Instant::now();
        ...
        let res = match self.client.ingest_sst(store_id, ingest).await {
            Ok(mut resp) => {
                if !resp.has_error() {
                    Ok(())
                } else {
